...
1、安装dapeng项目(dapeng-soa  2.0.1-SNAPSHOT)到本地仓库。或者进入dapeng项目的dapeng-code-generator 模块执行 mvn clean install ,再进入dapeng-maven-plugin模块执行 mvn clean install ，只安装这两个也行。
...
6、用插件生成的代码如果有依赖common-library的，必须用2.0.1-SNAPSHOT或以上的版本

7、使用这个插件如果有依赖dapeng 必须是2.0.1-SNAPSHOT或以上的版本