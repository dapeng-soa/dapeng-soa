          ...
          <div>
            ...
            {toFieldArrayBuffer(method.request.getFields).map{(field: Field)=>{
              <div>
                ...
                public void set{field.name.charAt(0).toUpper + field.name.substring(1)}({toFieldDeclareTemplate(field)} {field.name})<block>
        ...
        toMethodArrayBuffer(service.methods).map{(method: Method)=> {
          ...
          <div>
            ...
            <block>
              ...
              <block>
                CompletableFuture{lt}{ if(method.response.getFields.get(0).getDataType.kind== DataType.KIND.VOID) "Void" else  toDataTypeTemplate(method.response.getFields.get(0).getDataType)}{gt} result = (CompletableFuture{lt}{ if(method.response.getFields.get(0).getDataType.kind== DataType.KIND.VOID) "Void" else  toDataTypeTemplate(method.response.getFields.get(0).getDataType)}{gt}) iface.{method.name}({toFieldArrayBuffer(method.getRequest.getFields).map(i => {method.name} + "_args." + "get" + i.name.substring(0,1).toUpperCase() + i.name.substring(1) + "()").mkString(",")});
                return result.thenApply(({if(method.response.getFields.get(0).getDataType.kind== DataType.KIND.VOID)  "Void" else  <div> {toDataTypeTemplate(method.response.getFields.get(0).getDataType)} i</div>}) -> <block>
  ...
  def getJavaReadElement(dataType: DataType, index: Int):Elem = {
    dataType.kind match {
      ...
      case KIND.MAP => <div>com.github.dapeng.org.apache.thrift.protocol.TMap _map{index} = iprot.readMapBegin();
        java.util.Map{lt}{toDataTypeTemplate(dataType.keyType)},{toDataTypeTemplate(dataType.valueType)}{gt} elem{index} = new java.util.HashMap{lt}{gt}(_map{index}.size);
      ...
      case KIND.LIST => <div> com.github.dapeng.org.apache.thrift.protocol.TList _list{index} = iprot.readListBegin();
        java.util.List{lt}{toDataTypeTemplate(dataType.valueType)}{gt} elem{index} = new java.util.ArrayList{lt}{gt}(_list{index}.size);
      ...
      case KIND.SET => <div>com.github.dapeng.org.apache.thrift.protocol.TSet _set{index} = iprot.readSetBegin();
        java.util.Set{lt}{toDataTypeTemplate(dataType.valueType)}{gt} elem{index} = new java.util.HashSet{lt}{gt}(_set{index}.size);
    ...
    dataType.kind match {
      ...
      case KIND.LIST =>
        <div>
          ...
          for({toDataTypeTemplate(dataType.valueType)} elem{index+1} : elem{index})<block>
      ...
      case KIND.MAP =>
        <div>
          ...
          for(java.util.Map.Entry{lt}{toDataTypeTemplate(dataType.keyType)}, {toDataTypeTemplate(dataType.valueType)}{gt} _it{index} : elem{index}.entrySet())<block>

          {toDataTypeTemplate(dataType.keyType)} elem{index+1} = _it{index}.getKey();
          {toDataTypeTemplate(dataType.valueType)} elem{index+2} = _it{index}.getValue();
        ...
        if(field.dataType.getKind() == DataType.KIND.VOID) {
          ...
          if(field.isOptional){
            <div>if(bean.get{field.name.charAt(0).toUpper + field.name.substring(1)}().isPresent())<block>
              ...
              {toDataTypeTemplate(field.dataType)} elem{index} = bean.get{field.name.charAt(0).toUpper + field.name.substring(1)}().get();
            ...
            oprot.writeFieldBegin(new com.github.dapeng.org.apache.thrift.protocol.TField("{field.name}", {toThriftDateType(field.dataType)}, (short) {field.tag}));
            {toDataTypeTemplate(field.dataType)} elem{index} = bean.get{field.name.charAt(0).toUpper + field.name.substring(1)}();
            {toJavaWriteElement(field.dataType, index)}
            {index = index + 1}
            oprot.writeFieldEnd();
          </div>
