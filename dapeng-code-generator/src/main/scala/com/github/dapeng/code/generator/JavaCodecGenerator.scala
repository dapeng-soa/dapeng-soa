...
class JavaCodecGenerator extends CodeGenerator {
  ...
  def toSupperCodecTemplate(service:Service, namespaces:util.Set[String],structNamespaces:util.Set[String]): Elem = {
    return {
      <div>
  ...
  def toCodecTemplate(service:Service, namespaces:util.Set[String],structNamespaces:util.Set[String]): Elem = {
    ...
    return {
      <div>package {service.namespace.substring(0, service.namespace.lastIndexOf("."))};
        ...
        *  @generated
        **/
        public class {service.name}Codec extends {service.name}SuperCodec<block>
  ...
  def toAsyncCodecTemplate(service:Service, namespaces:util.Set[String],structNamespaces:util.Set[String]): Elem = {
    ...
    return {
      <div>package {service.namespace.substring(0, service.namespace.lastIndexOf("."))};
        ...
        *  @generated
        **/
        public class {service.name}AsyncCodec extends {service.name}SuperCodec<block>
        ...
        toMethodArrayBuffer(service.methods).map{(method: Method)=> {
          ...
          <div>
            ...
            <block>
              ...
              <block>
                CompletableFuture{lt}{ if(method.response.getFields.get(0).getDataType.kind== DataType.KIND.VOID) "Void" else  toJavaDataType(method.response.getFields.get(0).getDataType)}{gt} result = (CompletableFuture{lt}{ if(method.response.getFields.get(0).getDataType.kind== DataType.KIND.VOID) "Void" else  toJavaDataType(method.response.getFields.get(0).getDataType)}{gt}) iface.{method.name}({toFieldArrayBuffer(method.getRequest.getFields).map(i => {method.name} + "_args." + i.name).mkString(",")});
